...
function SingleDietPage() {
  ...
  const isMealAdded = userDiets.some(
    (userDiet) => userDiet.key === mealData._id
  );
  ...
  const handleRemoveButtonClick = async () => {
    try {
      ...
      await axios.delete(
        `http://localhost:8080/api/v1/goal/diet/${userDiet.goalId}`,
        {
          headers: {
            Authorization: `${token}`,
          },
            ...
            <div className="App" style={{ marginTop: "25px" }}>
              {mealData ? (
                <div className="container">
                  ...
                  <div className="text-container">
                    <div className="label">
                      <center id="dietSidebarAdjustment">
                        {mealData.name}
                      </center>
                    </div>
                    <h2>Ingredients</h2>
                    ...
                    <div className="button-container">
                      <button
                        ...
                        style={{
                          opacity: isMealAdded ? 0.5 : 1,
                          cursor: isMealAdded ? "not-allowed" : "pointer",
                        }}
                        onClick={() => setShowDietForm(true)}
                      >
                        Add
